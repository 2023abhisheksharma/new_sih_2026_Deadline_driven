...
import { loadSouthernLandRings, type LandPolygonRing } from '../services/landService';
import { calculateGeodesicDistanceMeters } from '../services/iceHazardService';
import { LayerHighlightToggle } from './LayerHighlightToggle';

interface Tactical2DViewProps {
  ports?: PortRecord[];
...
export const Tactical2DView: FC<Tactical2DViewProps> = ({
  ...
  // Load real landmass polygons once
  useEffect(() => {
    let active = true;
  ...
  const projectToScreen = useCallback(
    (lon: number, lat: number, width: number, height: number): [number, number] => {
      const centerLatRad = toRad(viewCenter.lat);
      const cosLat = Math.cos(centerLatRad);

      const dxMeters = toRad(lon - viewCenter.lon) * R * cosLat;
      const dyMeters = toRad(lat - viewCenter.lat) * R;
  ...
  const unprojectFromScreen = useCallback(
    (screenX: number, screenY: number, width: number, height: number): [number, number] => {
      ...
      const dLon = (dxMeters / (R * cosLat)) * (180.0 / Math.PI);
      const dLat = (dyMeters / R) * (180.0 / Math.PI);
  ...
  const nearbyUsnic = icebergs
    .map((berg) => {
      const dist = calculateGeodesicDistanceMeters(
        vesselLon,
        vesselLat,
        berg.longitude,
        berg.latitude
  ...
  useEffect(() => {
    ...
    ctx.rotate(toRad(vesselPoint.headingDegrees));
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const centerLatRad = toRad(dragStartRef.current.startLat);
    const cosLat = Math.cos(centerLatRad);

    const dLonMeters = -dx * zoomLevel;
    const dLatMeters = dy * zoomLevel;

    const newLon = dragStartRef.current.startLon + (dLonMeters / (R * cosLat)) * (180.0 / Math.PI);
    const newLat = dragStartRef.current.startLat + (dLatMeters / R) * (180.0 / Math.PI);
        ...
        <div className="bg-polar-950/90 rounded border border-polar-800 shadow-sm px-3 py-2 flex flex-col md:flex-row items-center justify-between gap-3 text-xs font-sans">
          ...
          <div className="flex-1 w-full flex items-center gap-2.5 font-sans text-xs text-slate-300">
            <span className="text-[11px] text-slate-400 shrink-0">
              {(vesselPoint.distanceTraveledMeters / 1852.0).toFixed(0)} NM
            </span>
            <input
              type="range"
              min={0}
            ...
            />
            <span className="text-[11px] text-slate-400 shrink-0">
              {(vesselPoint.totalDistanceMeters / 1852.0).toFixed(0)} NM
            </span>
