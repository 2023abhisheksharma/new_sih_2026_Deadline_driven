#!/usr/bin/env node
/**
 * Phase 8 — Final ANDSS Integration & End-to-End System Validation Suite
 * -----------------------------------------------------------------------
 * Evaluates the unified end-to-end routing workflow across all 17 authoritative routes:
 ...
 *
 * Verifies:
 * 1. Single authoritative route result dispatch (3D & 2D rendering identical coordinates).
 * 2. Strict land/ice validation enforcement (1.5 km micro-dock gate unchanged).
 * 3. Terminal harbor approach water-safe radial clearance.
 * 4. Multi-objective route quality ranking Q(R).
 * 5. Accurate failure taxonomy:
 *    - Cape Town: Validated hybrid route (0 land crossings)
 *    - Mumbai: Internal MARNET topology failure (mesh edge cuts land)
 *    - Sydney: Terminal harbor approach failure (berth in ria exceeds 5 km scan)
 *    - Patagonia: Patagonian isolated fjord (HYBRID_UNAVAILABLE)
 * 6. Dual-run 100% bit-for-bit determinism.
 * 7. Production artifact generation:
 *    - data/routing/processed/phase8_e2e_benchmark.json
 *    - data/routing/metadata/phase8_lineage.json
 */
...
console.log('================================================================================');
console.log('PHASE 8 — FINAL ANDSS INTEGRATION & END-TO-END VALIDATION SUITE');
console.log('================================================================================');
...
export function getAdaptiveDockToleranceKm(port) {
...
// Authoritative Hard Land Validation (Phase 3 invariant with Antimeridian & Adaptive Harbor Tolerance)
function checkLandIntersections(coordinates, rings, depToleranceKm = 1.5, destToleranceKm = 1.5) {
  ...
  for (let i = 0; i < numSegments; i++) {
    ...
    // Split antimeridian-crossing segments (|Δlon| > 180 deg) into spherical halves
    // to avoid spurious 2D Cartesian chord intersections across entire continents.
      ...
      for (let r = 0; r < rings.length; r++) {
        ...
        if (pointInPolygon(midPt, ring)) {
          if (isTerminal) {
            ...
            if (midDistFromDockKm <= dockToleranceKm) continue;
          }
          intersects = true;
          hitReason = isTerminal
...
function computePhase7Route(originPort, destPort) {
  ...
  const depTol = getAdaptiveDockToleranceKm(originPort);
  const destTol = getAdaptiveDockToleranceKm(destPort);
  ...
  if (originSnap && destSnap) {
    ...
    if (path && path.length > 0) {
      ...
      const fails = checkLandIntersections(full, landRings, depTol, destTol);
    ...
    try {
      ...
      if (rawRoute && rawRoute.geometry) {
        ...
        if (netCoords && netCoords.length >= 2) {
          const depApproach = findSafeTerminalApproach(origCoords, netCoords[0], true, landRings, depTol);
          const arrApproach = findSafeTerminalApproach(destCoords, netCoords[netCoords.length - 1], false, landRings, destTol);
          ...
          const fails = checkLandIntersections(fullCoords, landRings, depTol, destTol);
  ...
  if (originSnap && !destSnap) {
    ...
    for (const gw of candidates) {
      ...
      const termApproach = findSafeTerminalApproach(destCoords, lastMarnet, false, landRings, destTol);
      ...
      const fails = checkLandIntersections(stitched, landRings, depTol, destTol);
  ...
  if (!originSnap && destSnap) {
    ...
    for (const gw of candidates) {
      ...
      const termApproach = findSafeTerminalApproach(origCoords, firstMarnet, true, landRings, depTol);
...
const outBenchmarkPath = path.join(projectRoot, 'data/routing/processed/phase8_e2e_benchmark.json');
const outLineagePath = path.join(projectRoot, 'data/routing/metadata/phase8_lineage.json');
...
const benchmarkArtifact = {
  metadata: {
    benchmarkVersion: 'Phase 8 Final ANDSS Integration & End-to-End Release',
...
const lineageArtifact = {
  phase: 'PHASE_8_FINAL_ANDSS_INTEGRATION',
  timestamp: new Date().toISOString(),
  gitProvenance: {
    branch: 'phase8-final-integration',
  ...
  upstreamDependencies: {
    ...
    phase7: 'Terminal Harbor Approaches & Route Quality Model',
  },
  phase8Innovations: {
    singleAuthoritativeOwner: 'Unified dispatch flow between AntarcticOverview, CesiumGlobe, Tactical2DView, and RouteSummaryPanel',
    unifiedRouteStateModel: 'Strict operational distinction between VALID ROUTE, ROUTE CONSTRUCTED BUT REJECTED, and ROUTE UNAVAILABLE',
    preservedFailureTaxonomy: 'Surfaces Internal MARNET topology defect (Mumbai), Terminal approach limit (Sydney), and Patagonian isolated fjord without data loss',
    semanticIcebergCategorization: 'Rigorous separation of USNIC observed large icebergs, BYU/NIC historical drift tracks, and Sentinel-1 SAR grounded targets',
    assumedSpeedDurationStandard: 'Explicitly labels transit time as estimated duration at assumed 15 kn, preventing fake operational ETA confusion',
  },
